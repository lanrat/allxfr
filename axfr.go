...
func axfr(ctx context.Context, domain, nameserver string, ip net.IP) (int64, error) {
	startTime := time.Now()
	records, savedPath, err := axfrToFile(ctx, domain, ip, nameserver)
	if err == nil && records > 0 {
		...
		// Send webhook notification immediately after successful transfer
		if webhookClient != nil && webhookClient.NotifyEnabled() && !*dryRun {
			zoneName := strings.TrimSuffix(domain, ".")
			dateStr := time.Now().Format(time.DateOnly)
			result, notifyErr := webhookClient.PostDownloadNotify(ctx, zoneName, savedPath, dateStr)
			if notifyErr != nil {
				log.Printf("[%s] webhook notification error: %v", domain, notifyErr)
			} else if result != nil && result.Status == "error" {
				log.Printf("[%s] webhook notification returned error: %s", domain, result.Message)
			} else {
				v("[%s] webhook notification sent (status: %s)", domain, result.Status)
			}
		}
	}
	return records, err
...
// envelope of records. Returns the number of records transferred, the intended file path,
// and any error. The file path is only meaningful when records > 0 and err == nil.
// Returns -1 records if the file already exists and overwrite is disabled.
func axfrToFile(ctx context.Context, zone string, ip net.IP, nameserver string) (int64, string, error) {
	zone = dns.Fqdn(zone)
	...
	return zonefile.Records(), filename, err
}
